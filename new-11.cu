...
//ZAD1
__global__ void copy(float *A,  float *B,  int numElements)
{
...
__global__ void scal(float *A, float b, int numElements)
{
    int i = blockDim.x * blockIdx.x + threadIdx.x;
...
__global__ void axpy(float *A, float x, float *B, int numElements)
{
    int i = blockDim.x * blockIdx.x + threadIdx.x;
...
//ZAD2
__global__ void sparse(int N, float *d_val, int *d_row, int *d_col, float *d_Ax, float *d_x)
{
    int i = blockDim.x * blockIdx.x + threadIdx.x;
...
void cgs_TODO(int argc, char **argv, int N, int M){
    ...
    */
    ...
    sparse<<<blocksPerGrid, threadsPerBlock>>>(N, d_val, d_row, d_col, d_Ax, d_x);
 
    //azpy: d_r = d_r + alpham1 * d_Ax
    //cublasSaxpy(cublasHandle, N, &alpham1, d_Ax, 1, d_r, 1);                              // PODMIEN FUNCKJE (ZADANIE-I)
    axpy<<<blocksPerGrid, threadsPerBlock>>>(d_Ax, alpham1, d_r, N);
    //dot:  r1 = d_r * d_r
    ...
    while (r1 > tol*tol && k <= max_iter)
    {
        ...
        sparse<<<blocksPerGrid, threadsPerBlock>>>(N, d_val, d_row, d_col, d_Ax, d_p);
